...
const ApprovedBlogs = (props) => {
	return (
		<>
			<div className="w-full text-center my-5 mb-10">
				<p className="text-2xl font-extrabold uppercase dark:bg-white bg-black bg-clip-text text-transparent bg-gradient-to-r from-[#ffe32d] to-[#ff6d2d] transition-all duration-300 w-fit mx-auto">All Approved Blogs</p>
			</div>
						...
						>
							<div className="absolute right-4 bottom-4 flex flex-col gap-y-3 bg-white shadow-xl dark:shadow-[#F05225] dark:shadow-md dark:bg-[#101010] p-2 rounded-lg translate-x-[200%] group-hover:translate-x-0 transition-all duration-300">
								...
								<svg
									...
									viewBox="0 0 16 16"
									onClick={() => {
										document.getElementById("approvedBlog-" + record.id).style.transform = "scale(0.1)"
										document.getElementById("approvedBlog-" + record.id).style.opacity = "0"
										setTimeout(() => {
											document.getElementById("approvedBlog-" + record.id).classList.add("hidden")
										}, 300)
									}}
								>

...
const UnderReview = (props) => {
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get(
        process.env.REACT_APP_SERVER + "/admin_use/"
      );
  ...
  const acceptBlog = async (slug) => {
    try {
      const response = await axios.post(
        `${process.env.REACT_APP_SERVER}/admin_use/request-accepet/${slug}`
      );

      // Handle success (e.g., update state, show a success message)
      console.log("Blog accepted successfully", response.data);

      // Optionally, you may want to refetch the data after accepting the blog
      fetchData();
    } catch (error) {
    ...
    <>
      <div className="w-full text-center my-5 mb-10">
        <p className="text-2xl font-extrabold uppercase dark:bg-white bg-black bg-clip-text text-transparent bg-gradient-to-r from-[#ffe32d] to-[#ff6d2d] transition-all duration-300 w-fit mx-auto">
          All Under Review Blogs
        </p>
      </div>
      <a className="my-5 flex flex-row flex-wrap items-center justify-center gap-x-10 gap-y-5 lg:px-20 px-10 w-full">
            ...
            >
              <div className="absolute right-4 bottom-4 flex flex-col gap-y-3 bg-white shadow-xl dark:shadow-[#F05225] dark:shadow-md dark:bg-[#101010] p-2 rounded-lg translate-x-[200%] group-hover:translate-x-0 transition-all duration-300">
                ...
                <svg width={20} height={20} viewBox="0 0 22 22" fill="none">
                  <path
                    d="M9.97124 0.905273H6.07645C2.87332 0.905273 0.86499 3.17298 0.86499 6.3834V15.0438C0.86499 18.2542 2.86395 20.5219 6.07645 20.5219H15.2681C18.4817 20.5219 20.4806 18.2542 20.4806 15.0438V10.848"
                    stroke="#37A945"
                    strokeWidth={1.5}
                    strokeLinecap="round"
                  ...
                  <path
                    fillRule="evenodd"
                    clipRule="evenodd"
                    d="M7.19565 9.37617L14.98 1.5918C15.9498 0.623047 17.5217 0.623047 18.4915 1.5918L19.7592 2.85951C20.729 3.8293 20.729 5.40221 19.7592 6.37096L11.9373 14.1928C11.5134 14.6168 10.9384 14.8553 10.3384 14.8553H6.43628L6.5342 10.9178C6.54878 10.3387 6.78524 9.78659 7.19565 9.37617Z"
                    stroke="#37A945"
                    strokeWidth={1.5}
                    strokeLinecap="round"
                    strokeLinejoin="round"
                  />
                  <path
                    d="M13.7971 2.79492L18.5534 7.55117"
                    stroke="#37A945"
                    strokeWidth={1.5}
                    strokeLinecap="round"
                    strokeLinejoin="round"
                ...
                <svg
                  ...
                  viewBox="0 0 16 16"
                  onClick={() => deleteBlog(blogData[index]["fields"]["slug"])}
                >
                ...
                <svg
                  ...
                  viewBox="0 0 16 16"
                  onClick={() => acceptBlog(blogData[index]["fields"]["slug"])}
                >
              ...
              <div class="w-[326px] h-fit p-6 rounded-md">
                ...
                <div className=" text-orange-400 font-semibold text-end ">
      ...
      {/* <div className="blog-upload-home">
		    ...
		    <tr className=' text-black border-b-[black] border-b border-solid'>
		        ...
		        {demoData.map((blog, index) => (
		            <tr className=' text-[white] text-black border-b-[black] border-b border-solid'>
